...
public class Contacts extends BaseContacts implements Cloneable {
    ...
    private int mMatchStartIndex; // the match start position of mMatchKeywords
    // in original string(name or phoneNumber).
    private int mMatchLength; // the match length of mMatchKeywords in original
    // string(name or phoneNumber).
    private boolean mSelected; // whether select contact
    private boolean mFirstMultipleContacts;// whether the first multiple
    // Contacts
    private boolean mHideMultipleContacts; // whether hide multiple contacts
    private boolean mBelongMultipleContactsPhone; // whether belong multiple
    // contacts phone, the value
    // of the variable will not
    // change once you set.

    private boolean mHideOperationView; // whether hide operation view
    private Contacts mNextContacts; // point the contacts information who has
    // multiple numbers.
	...
	/*
     * public static Comparator<List<Contacts>> mAscComparator = new
	 * Comparator<List<Contacts>>() {
	 * 
	 * @Override public int compare(List<Contacts> lhs, List<Contacts> rhs) {
	 * if((null==lhs)||(lhs.size()<=0)||(null==rhs)||(rhs.size()<=0)){ return 0;
	 * } return mChineseComparator.compare(lhs.get(0).mSortKey,
	 * rhs.get(0).mSortKey); } };
	 */
	 ...
	 * Log.i(TAG,"mId=["+getId()+"]mSortKey=["+mSortKey
	 * +"]"+"mName=["+getName()+"]+"
	 * +"mPhoneNumber:"+getPhoneNumber()+"+ phoneNumberCount=["
	 * +mMultipleNumbersContacts.size()+1+"]"); for(Contacts
	 * contacts:mMultipleNumbersContacts){ Log.i(TAG,
	 * "phone=["+contacts.getPhoneNumber()+"]"); } }
	 */
}
