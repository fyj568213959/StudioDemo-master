    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (tabCount > 0) {
            ...
            if (selectionOffset > 0f && selectedPosition < (getChildCount() - 1)) {
                ...
                left = (int) (leftOffset * Utils.getLeft(nextTab, indicatorWithoutPadding) + (1.0f - leftOffset) * left);
                right = (int) (rightOffset * Utils.getRight(nextTab, indicatorWithoutPadding) + (1.0f - rightOffset) * right);
                thickness = thickness * thicknessOffset;
            }

            drawIndicator(canvas, left + 18, right - 18, height, thickness, color);
...
//        drawSeparator(canvas, height, tabCount);
    ...
    private void drawIndicator(Canvas canvas, int left, int right, int height, float thickness, int color) {
        ...
        switch (indicatorGravity) {
            ...
            case GRAVITY_CENTER:
                center = height / 2f;
                top = center - (thickness / 2.5f);
                bottom = center + (thickness / 2.5f);
                break;
            case GRAVITY_BOTTOM:
            default:
